...
const QUESTIONS_PLACEHOLDER = `[
  {
    ...
    "text": "Question text",
    "options": [
      { "id": "a", "text": "Answer A", "value": 1 },
      { "id": "b", "text": "Answer B", "value": 2 }
    ]
  }
]`;
...
const SCORING_PLACEHOLDER = `{
  "ranges": [
    { "min": 0, "max": 5, "result": "Low" },
    { "min": 6, "max": 10, "result": "Medium" }
  ]
}`;
...
export function parseQuestionsJson(raw: string): { data?: TestQuestion[]; error?: string } {
  ...
  if (!trimmed) return { error: "Questions JSON is empty." };

  let parsed: unknown;
  try {
    parsed = JSON.parse(trimmed);
  } catch {
    return { error: "Questions JSON is malformed." };
  }
  ...
  for (let i = 0; i < parsed.length; i++) {
    const q = parsed[i];
    if (!isRecord(q)) return { error: `Question ${i + 1}: must be an object.` };
    if (typeof q.id !== "string" || !q.id.trim()) {
      return { error: `Question ${i + 1}: id (string) is required.` };
    }
    if (typeof q.text !== "string" || !q.text.trim()) {
      return { error: `Question ${i + 1}: text (string) is required.` };
    }
    if (!Array.isArray(q.options) || q.options.length === 0) {
      return { error: `Question ${i + 1}: options must be a non-empty array.` };
    }

    const options: TestQuestion["options"] = [];
    for (let j = 0; j < q.options.length; j++) {
      const o = q.options[j];
      if (!isRecord(o)) return { error: `Question ${i + 1}, option ${j + 1}: must be an object.` };
      if (typeof o.id !== "string" || !o.id.trim()) {
        return { error: `Question ${i + 1}, option ${j + 1}: id (string) is required.` };
      }
      if (typeof o.text !== "string" || !o.text.trim()) {
        return { error: `Question ${i + 1}, option ${j + 1}: text (string) is required.` };
      }
      if (typeof o.value !== "number" || Number.isNaN(o.value)) {
        return { error: `Question ${i + 1}, option ${j + 1}: value (number) is required.` };
      }
...
export function parseScoringRulesJson(raw: string): { data?: TestScoringRules; error?: string } {
  const trimmed = raw.trim();
  if (!trimmed) return { error: "Scoring JSON is empty." };

  let parsed: unknown;
  try {
    parsed = JSON.parse(trimmed);
  } catch {
    return { error: "Scoring JSON is malformed." };
  }

  if (!isRecord(parsed)) {
    return { error: "Scoring JSON must be an object." };
  }

  // Big Five format
  if (parsed.type === "big_five") {
    if (!isRecord(parsed.traits)) {
      return { error: "big_five: traits object is required." };
    }
    if (!Array.isArray(parsed.resultLevels) || parsed.resultLevels.length === 0) {
      return { error: "big_five: resultLevels array is required." };
    }
  ...
  if (parsed.type === "category_count") {
    if (!isRecord(parsed.optionValueToCategory)) {
      return { error: "category_count: optionValueToCategory object is required." };
    }
    if (!Array.isArray(parsed.categoryOrder) || parsed.categoryOrder.length === 0) {
      return { error: "category_count: categoryOrder array is required." };
    }
    if (!isRecord(parsed.categories)) {
      return { error: "category_count: categories object is required." };
    }
    return { data: parsed as unknown as TestScoringRules };
  }
  ...
  if (!Array.isArray(parsed.ranges) || parsed.ranges.length === 0) {
    return { error: "scoring_rules.ranges must be a non-empty array." };
  }

  const ranges: { min: number; max: number; result: string }[] = [];
  for (let i = 0; i < parsed.ranges.length; i++) {
    const r = parsed.ranges[i];
    if (!isRecord(r)) return { error: `Range ${i + 1}: must be an object.` };
    if (typeof r.min !== "number" || Number.isNaN(r.min)) {
      return { error: `Range ${i + 1}: min (number) is required.` };
    }
    if (typeof r.max !== "number" || Number.isNaN(r.max)) {
      return { error: `Range ${i + 1}: max (number) is required.` };
    }
    if (typeof r.result !== "string" || !r.result.trim()) {
      return { error: `Range ${i + 1}: result (string) is required.` };
    }
