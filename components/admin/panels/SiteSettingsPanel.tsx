...
export function SiteSettingsPanel() {
  ...
  function uploadIntroVideo() {
    if (!videoFile) {
      toast("Танилцуулгын видео сонгоно уу.", "error");
      return;
    }
    startTransition(async () => {
      setStatus("Видео байршуулж байна…");
      const presign = await presignIntroVideoUpload(videoFile.type);
      if (presign.error || !presign.uploadUrl) {
        toast(mapServerErrorToMn(presign.error ?? "Алдаа"), "error");
        setStatus("");
        return;
      }
      ...
      if (!put.ok) {
        toast("Видео байршуулахад алдаа.", "error");
        setStatus("");
        return;
      }
      ...
      setVideoFile(null);
      toast("Танилцуулгын видео хадгалагдлаа", "success");
    });
  }

  function uploadPoster() {
    if (!posterFile) {
      toast("Постер зураг сонгоно уу.", "error");
      return;
    }
    startTransition(async () => {
      setStatus("Постер байршуулж байна…");
      ...
      setPosterFile(null);
      toast("Постер хадгалагдлаа", "success");
    });
  }
  ...
  return (
    <section style={{ maxWidth: 560, display: "flex", flexDirection: "column", gap: 28 }}>
      <div>
        <div className="u-eyebrow">Танилцуулга</div>
        <h2 style={{ font: "var(--u-h3)", margin: "8px 0 0" }}>Нүүр хуудасны видео</h2>
        <p style={{ font: "var(--u-body-s)", color: "var(--u-ink-3)", margin: "8px 0 16px" }}>
          R2 түлхүүр: <code style={{ fontFamily: "var(--u-mono)" }}>site/intro.mp4</code>
          {introVideoR2Key ? (
            <>
              <br />
              Одоогийн: {introVideoR2Key}
            </>
          ) : null}
        </p>
      ...
      <div style={{ borderTop: "1px solid var(--u-rule)", paddingTop: 24 }}>
        <h3 style={{ font: "var(--u-h4)", margin: "0 0 8px" }}>Видеоны постер</h3>
        {introPosterPath ? (
          <p style={{ font: "var(--u-body-s)", color: "var(--u-ink-3)", margin: "0 0 12px" }}>
            Одоогийн: {introPosterPath}
          </p>
        ) : null}
        <label style={{ display: "block", font: "var(--u-body-s)", marginBottom: 8 }}>
          Постер (JPEG/PNG/WebP, 3MB)
          <input
            type="file"
